...
describe PersistentTree::Map do
    ...
    context 'when a block is provided' do
      it 'returns itself once iteration is complete' do
        (lower_limit..upper_limit).each do |value|
          ...
          expect(view.send(method_name) { |unused| unused }).to be(view)
        end
      end
  ...
  describe '.new' do
    context 'with a map-level default value lambda with an incorrect number of arguments' do
      it 'returns cause an error to be raised' do
  ...
  describe '#[]' do
    context 'when called on a map containing two associations, but no map-level default value' do
      ...
      context 'with one argument, for a key that exists' do
        it 'retrieves the value previously associated with a key' do
    ...
    context 'when called on a map containing two associations with a map-level default value' do
      ...
      context 'with one argument, for a key that does not exist' do
        it 'returns the Map-level default value' do
    ...
    context 'when called on an empty map' do
      context 'with a map-level default value lambda with the correct number of arguments' do
        ...
        it 'returns the value returned by the lambda' do
      ...
      context 'with a map-level default value block with the correct number of arguments' do
        ...
        it 'returns the value generated by the block' do
      ...
      context 'with a map-level default value that is actually a proc' do
        ...
        it 'returns the proc instead of the value that it would generate if called' do
    ...
    context 'when called on a map containing two associations with a map-level default value block' do
      ...
      context 'with one argument, for a key that does not exist' do
        it 'returns the value generated by the Map-level default value block' do
  ...
  describe '#[]=' do
    it 'allows values to be associated with new keys' do
      expect((map['a'] = '1').to_s).to eq('1')
      expect((map['b'] = '4').to_s).to eq('4')
    end

    it 'allows values to be replace those associated with existing keys' do
      expect((map['a'] = '1').to_s).to eq('1')
      expect((map['a'] = '2').to_s).to eq('2')
    end

    it 'freezes unfrozen strings used as keys' do
      a = 'a'.dup
      expect(a).not_to be_frozen
      expect((map[a] = '1').to_s).to eq('1')
      expect(map.keys[0]).to be_frozen
    end
  end
  ...
  describe '#each_key' do
    ...
    context 'when a block is provided' do
      it 'returns itself once iteration is complete' do
        (lower_limit..upper_limit).each do |value|
          view = map.version(value)
          expect(view.each_key { |unused| unused }).to be(view)
        end
      end
  ...
  describe '#fetch' do
    ...
    context 'when a default value is provided' do
      it 'retrieves the values previously associated with keys' do
        expect(map.fetch('a', '3')).to eq('1')
        expect(map.fetch('b', '3')).to eq('4')
      end

      it 'returns the default value when fetching a value for a non-existent key' do
    ...
    context 'when a code block is provided' do
      ...
      it 'defers to the block when fetching a value for a non-existent key' do
    ...
    context 'when both a default value and a code block is provided' do
      ...
      it 'produces a warning about superseded default value argument' do
        map.fetch('a', '3', &block)
        $stderr.rewind
    ...
    context 'when neither a default value nor a code block is provided' do
      ...
      it 'raises an error when fetching a value for a non-existent key' do
  ...
  describe '#keys' do
    ...
    context 'on a map containing two entries' do
      ...
      it 'returns the keys belonging to those two entries' do
        keys = map.keys
      ...
      it 'yields correct list of keys at subsequent versions' do
        (lower_limit..upper_limit).each do |outer|
          ...
          (lower_limit..outer).each do |inner|
            expect(keys).to include("key-#{inner}")
          end
        end
      end
    end
  ...
  describe '#select' do
    ...
    context 'when a block is provided' do
      it 'returns a new map once iteration is complete' do
        (lower_limit..upper_limit).each do |value|
          view = map.version(value)
          expect(view.select { |_, _| true }).to be_a(described_class)
        end
      end
  ...
  describe '#store' do
    ...
    it 'clones and freeze unfrozen strings used as keys' do
      a = 'a'.dup
      expect(a).not_to be_frozen
      expect(map.store(a, '1').to_s).to eq('1')
      expect(map.keys[0]).to be_frozen
  ...
  describe '#version' do
    ...
    context 'when called on a new map' do
      context 'with no arguments' do
        it 'returns a version number of zero' do
          expect(map.version).to eq(0)
        end
      end

      context 'with one integer argument' do
        it 'returns a read-only view when version zero is requested' do
          expect(map.version(0).size).to eq(0)
        end

        it 'raises an error if a version other than zero is requested' do
    ...
    context 'when called on a map after two insertions' do
      ...
      context 'with no arguments' do
        it 'returns a version of 2' do
          expect(map.version).to eq(2)
        end
      end

      context 'with one argument' do
        it 'returns a read-only view of the map at version 0 that reflects an empty map' do
          view = map.version(0)
          expect(view.size).to eq(0)
        end

        it 'returns a read-only view of the map at version 1 that reflects only the first insertion' do
          view = map.version(1)
          expect(view.fetch('a')).to eq('1')
          expect { view.fetch('b') }.to raise_error(KeyError)
        end

        it 'returns a read-only view of the map at version 2 that reflects both insertions' do
          view = map.version(2)
          expect(view.fetch('a').to_s).to eq('1')
          expect(view.fetch('b').to_s).to eq('4')
        end

        it 'raises an error if a version greater than 2 is requested' do
